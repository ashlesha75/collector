...
class Admin_Dashboard_Model extends CI_Model
{
    ...
    {
        parent::__construct();
        if (!$this->session->userdata('adid') && !$this->session->userdata('id'))
            redirect('admin/login');
    }



    public function get_rahivasi_form_details($id = NULL, $verifycertificate = NULL)
    {
        ...
        if ($adid['admin_type'] == 'po' && $verifycertificate == NULL) {
            $this->db->where('clerk_verify_dsc', '1');
    ...
    public function get_approved_application_details($id = NULL, $barcode = NULL, $not_verify_dsc_code = NULL)
    {
        //$this->db->select('rahivasi_form.*,talukas.taluka_name,district_name');
        $adid = $this->session->userdata('adid');
        $this->db->select('*');
        $this->db->from('rahivasi_form');
        $id != '' ? $this->db->where('r_id', $id) : "";
        $barcode != '' ? $this->db->where('dsc_code', $barcode) : "";
        $this->db->where('po_verify', '1');
        if ($not_verify_dsc_code) {
            $this->db->where('send_dsc', '0');
        }
        $this->db->order_by('send_dsc');
        $query = $this->db->get();
    ...
    public function update_registered_users_complaint($remark_id,$remarks){
        $data = array(
            'clerk_remark' => $remarks,
        );
        $this->db->where('c_id', $remark_id);
